...
def get_revenue_data(ticker):
    try:
        ...
        revenue["Date"] = pd.to_datetime(revenue["Date"]).dt.tz_localize(None)
...
def get_netprofit_data(ticker):
    try:
        ...
        margin_df["Date"] = pd.to_datetime(margin_df["Date"]).dt.tz_localize(None)
...
def correlation(stock_data, revenue_data):
    ...
    merged = pd.merge(quarterly_stock, revenue_data, on="Quarter", how="inner")

    if len(merged) < 4:
        return None

    correlation = merged["Stock Price"].corr(merged["Revenue"])
...
def plot_stock_ma(stock_data, ticker):
    ...
    fig.update_layout(
        title=f"{ticker.upper()} Stock Price with Moving Averages",
        xaxis_title="Date",
...
def plot_revenue(revenue_data, ticker):
    ...
    fig.update_layout(
        title=f"{ticker.upper()} Quarterly Revenue",
        xaxis_title="Date",
...
def plot_comparison(stock_data_1, stock_data_2, ticker1, ticker2):
    "Normalize both stocks and compare"
    fig = go.Figure()
    ...
    fig.update_layout(
        title=f"Normalized Price Comparison: {ticker1.upper()} vs {ticker2.upper()} (Base = 100)",
        xaxis_title="Date",
...
if st.sidebar.button("Generate Report"):
    ...
    try:
        ...
        if test.get("lastPrice") is None:
            st.error(f"'{ticker}' doesn't appear to be a valid ticker.")
            st.stop()
    except Exception:
    ...
    if ticker2 and ticker2 != ticker:
        st.subheader(f"Stock Comparison: {ticker} vs {ticker2}")

        with st.spinner(f"Fetching data for {ticker2}..."):
            stock_data_2 = get_stock_data(ticker2, start, end)

        if stock_data_2 is not None and not stock_data_2.empty:
            st.plotly_chart(
                plot_comparison(stock_data, stock_data_2, ticker, ticker2),
                use_container_width=True,
            )

            # Comparison metrics
            return_1 = pct_change(
                stock_data["Stock Price"].iloc[0], stock_data["Stock Price"].iloc[-1]
            )
            return_2 = pct_change(
                stock_data_2["Stock Price"].iloc[0],
                stock_data_2["Stock Price"].iloc[-1],
            )
            vol_1 = volatility(stock_data)
    ...
    if len(recent_stock) >= 2:
        ...
        insights.append(
            f"**Stock Price (last 3 years):** {ticker} has {direction} by "
    ...
    if vol_val is not None:
        ...
        insights.append(
            f"**Volatility:** {ticker} has an annualized volatility of "
    ...
    if revenue_data is not None and len(revenue_data) >= 8:
        ...
        if rev_growth is not None:
            ...
            insights.append(
                f"**Revenue Growth:** {ticker} has demonstrated {rev_label} in quarterly "
    ...
    if corr_val is not None:
        ...
        insights.append(
            ...
            f"quarterly stock price and revenue for {ticker}, suggesting {corr_meaning}."
        )
    ...
    if volatility_summary is not None:
        ...
        st.markdown(
            f"**Summary:** Based on the selected period, {ticker} appears to be a "
